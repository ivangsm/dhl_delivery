...
class delivery:
    ...
    def get_delivery(self, dict_param):
        return_status = True
        return_message = ''
        ...
        dimensional_weight = ''
        insured_amount = ''
        delivery_date = '' # ie pickup or dropoff to dhl date, not the date when its being created
        ...
        xml_formated_data = self.delivery_xml(dict_param)
        ...
        try:
            ...
            returned_data = dhl_process_obj.process_delivery(xml_formated_data_string)
            ...
            if 'Success' in action_note: # action_note is a list type
                return_status = True
                return_message = dhl_delivery.config.message_delivery_success
                # set pick up - if pickup provided
                if 'pickup_details' in dict_param:
                    if len(dict_param['pickup_details']) > 0:
                        request_for_pickup = True
                        pickup_returned_data = dhl_delivery.pickup.schedule_pickup(dict_param)
                ...
                # delivery Date ie pickup or dropoff to dhl, not the date when its being created
                delivery_date = common_obj.in_dictionary_all('deliveryDate', dict_response) 
                delivery_date = ''.join(map(str, delivery_date))
            ...
            if dhl_delivery.dhl_response_flag:
                return_dict.update({'dhl_response': {'dhl_delivery_response': dict_response}})
                if 'dhl_response' in pickup_returned_data.keys():
                    return_dict.update({'dhl_response': {'dhl_delivery_response': dict_response, 'dhl_pickup_response': pickup_returned_data['dhl_response']}})
    ...
    def create_pdf_awb(self, dict_param, dict_response, airway_bill_number=''):
        ...
        if 'optional_data' in dict_param.keys():
            ...
            if not awb_pdf_file_path:
                awb_pdf_file_path = os.path.join('pdf_delivery_label_default')
    ...
    def delivery_xml(self, dict_param):
        try:
            ...
            xml_str += '<req:deliveryValidateRequestAP xmlns:req="http://www.dhl.com" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" '
            ...
            # Billing
            xml_str += '<Billing>'
            xml_str += '<ShipperAccountNumber>' + dhl_delivery.dhl_account_no + '</ShipperAccountNumber>'
            xml_str += '<deliveryPaymentType>' + dhl_delivery.delivery_payment_type + '</deliveryPaymentType>'
            xml_str += '<DutyPaymentType>' + dhl_delivery.duty_payment_type + '</DutyPaymentType>'
            xml_str += '</Billing>'
            # Billing ENDS
            ...
            xml_str += '<Date>' + str(package['delivery_date']) + '</Date>'  # delivery date for when package(s) will be shipped (but usually current date)  ' + self.date_now.strftime("%Y-%m-%d") + '
            ...
            xml_str += '</req:deliveryValidateRequestAP>'
